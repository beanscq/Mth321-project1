...
u^{(k+1)}&=u^{(k)}+\Delta u^{(k)}.
\end{align*}
The stability function $R_{IE}(z)=1/(1-z)$ contains the left half-plane. The final code must state the predictor, residual and update norms, stopping condition, maximum iteration count, and failed-step policy. These implementation details are \pending{} until the final code is provided; the report will not invent them.
